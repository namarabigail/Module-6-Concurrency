...
## Bonus Reflection: `new` vs `build`
Pada bonus ini, saya merefactor pembuatan `ThreadPool` dengan membuat fungsi `build` sebagai alternatif dari `new`. Pada implementasi awal menggunakan `new`, kita menggunakan `assert!(size > 0)`. Hal ini memiliki kelemahan yang fatal, di mana jika argumen `size` yang diberikan adalah 0, program akan langsung memicu *panic* dan server akan berhenti secara mendadak. Dengan mengimplementasikan `build`, kita mengubah *return type* dari yang awalnya langsung berupa `ThreadPool` menjadi tipe `Result<ThreadPool, PoolCreationError>`. Jika `size` tidak valid (0), fungsi akan me-return `Err` tanpa menghentikan program di level *library*. Kontrol penanganan error diserahkan sepenuhnya kepada caller (`main.rs`), sehingga program dapat memberikan pesan *error* yang jelas.